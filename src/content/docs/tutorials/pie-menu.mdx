...
2. Something to browser the contents of XML files. My preferred tool for this is Doc Fetcher. Other options include FileSeek or whatever works for you.
3. Sims 4 Studio
...
2. A new window will pop up that you can search in.
   ...
   None of these is what I want. There are some there that *might* work, but I'd need to check exactly what they do and how they're set up. I'm pretty sure the basic Friendly pie menu already does exactly what I want.

   So this didn't work. Off to the program that searches the contents of XML files we go (Doc Fetcher for me).

3. Here in Doc Fetcher, I've already [extracted all the tuning files](../xml-extractor) and built an index so that it can search.
   ...
   Good, but a lot. So we need to narrow it down.

4. We know we need a pie menu file. There are a few ways to find that.
   ...
   That narrows it down to nineteen files.
   
   ![Doc Fetcher's search results sorted by Path, with the files in the `pie_menu_category` folder highlighted.](~/assets/doc-fetcher-sorted-amethyst.png)

5. Most of these are obviously not right. They're subcategories of Friendly, and if that's what we wanted, that's what we'd clone. But we want a top level pie menu, so we want Friendly itself.

   ![The same image as above, with two files in a red box.](~/assets/doc-fetcher-sorted-friendly-amethyst.png)

   That leaves us with two pie menus that it could be, `Sim_BeFriendly` and `Sim_Friendly`.

   Again, there are different ways to figure out the right one. You could search both, one at a time, using the exact file name with no asterisks, and see which other files reference it. You could look at the interactions that are shown in the search results. You could use Lumpinou's EA Tuning Reference Browser on [Mod Hound](https://app.ts4modhound.com/).

   Since we have it open, I'll use Doc Fetcher, and from personal preference, I'll search both individually.
...
1. Here we search for Sim_Friendly.
...
2. The first thing to do is rename the file. Otherwise we'll be overriding the existing Friendly pie menu, and we're making a new one.
...
5. Next is Display Name.

   Most of the time, this is something you'll want to change. You can use text from the game, or you can create custom strings.

   To select a string from the game (or something in a file you imported), click the link symbol next to the box in this line. If you hover over the link, it says Select Reference. Pick whichever one you want for your pie menu's display name in game.
...
6. Next is Display Priority. This is how high your pie menu will appear in the main pie menu in game. What you put here is up to you. Lower numbers mean lower priority. If you always want it to be as far into the pie menu as possible, you can set it to 0.

7. Next is Icon. This is the symbol that will show at the beginning of the pie menu bubble in game.
...
9. Next is Parent. This is where you'd pick a pie menu category that your pie menu would be inside.
...
10. Lastly, there's Special Category. By default, this is No Category, and most of the time, that's the way it should be left.
    ...
    For a basic pie menu, you can leave this alone and unchecked. It doesn't need to be included.
